...
Combined Astra retrieval, live web evidence, prompting, Groq synthesis, and chat output. It was retired as StockSage evolved into a typed conversation pipeline with deterministic policy checks, bounded evidence extraction, parallel retrieval, citation validation, and cross-provider composition failover.
...
The application now serves stored market intelligence immediately and publishes stale or missing work to a signed background queue. Showcase tickers are refreshed every 30 minutes; other tickers are refreshed on demand. Failed provider or model work never removes the last usable result.
...
See the [current architecture](../docs/architecture.md) and [data pipeline](../docs/data-pipeline.md) for the implemented system.