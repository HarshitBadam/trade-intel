...
| AI and retrieval | `GROQ_*`, `TAVILY_API_KEY`, `LANGFLOW_*`, `STOCKSAGE_DEEP_SNAPSHOT_SECRET` | Regular chat synthesis and web context, plus Deep Research and batch analysis. The optional dedicated snapshot secret falls back to `AUTH_SECRET` or `NEXTAUTH_SECRET`. |
