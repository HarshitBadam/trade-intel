...
export function slidingLimiter(
  ...
  // waiters as the budget allows, then re-arm one timer for the next moment
  // capacity frees up. Re-checking the budget on every wake keeps the release
  // bounded no matter how many callers piled up.
  const pump = () => {
    const now = Date.now();
    prune(now);
