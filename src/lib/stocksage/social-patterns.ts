export const SOCIAL =
  /^(?:(?:yo+|hey|hi|aight|ok(?:ay)?)[\s,]+)?(?:(?:hey|hi|hello|hiya|howdy|yo+|sup|wass?up|g'?day|good\s+(?:morning|afternoon|evening))(?:\s+again)?(?:,\s*i'?m back)?(?:[,\s]+(?:boss|bro|mate|dude|there|sage|stocksage))?|i'?m back|how are you|how'?s it going(?:[,\s]+(?:boss|bro|mate|dude))?|what'?s (?:up|good|new|happening)(?:[,\s]+(?:boss|bro|mate|dude))?|nice to meet you|aight(?:\s+gucci)?(?:\s+then)?|gucci|all good(?:\s+then)?|no worries|cool|sounds good|okay|ok|thx|thanks?(?:,\s*that helps|\s+(?:boss|bro|mate|dude))?|thank you(?:\s+(?:boss|bro|mate|dude))?|cheers(?:\s+(?:boss|bro|mate|dude))?|much appreciated|that helps|got it|gotcha)[\s,.!?…-]*$/i;

export const FAREWELL =
  /^(?:(?:thanks?|thank you|thx|cheers|ok(?:ay)?|aight)[\s,!.]*)?(?:bye|goodbye|see (?:you|ya)|peace(?:\s+out)?|sayonara|later[sz]?|catch (?:you|ya)(?:\s+later)?|ciao|cya|ttyl|take care|good\s*night|i'?m (?:off|out|done(?:\s+for\s+(?:today|the\s+day))?)|gtg|gotta (?:go|run|bounce)|all good then|that'?s all)(?:[\s,]+(?:for now|for today|later|soon|again|then|boss|bro|mate|dude|thanks|thank you|sage))*[\s,.!?…-]*$/i;
