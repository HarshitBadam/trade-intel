...
export const FAREWELL =
  /^(?:(?:thanks?|thank you|thx|cheers|ok(?:ay)?|aight|gucci|all good(?:\s+then)?)[\s,!.]*)?(?:bye|goodbye|farewell|see (?:you|ya)|peace(?:\s+out)?|sayonara|later[sz]?|catch (?:you|ya)(?:\s+later)?|ciao|cya|ttyl|take care|good\s*night|adios|au revoir|auf wiedersehen|hasta luego|arrivederci|alvida|phir milenge|zai\s*jian|i'?m (?:off|out|done(?:\s+for\s+(?:today|the\s+day))?)|gtg|gotta (?:go|run|bounce)|all good then|that'?s all)(?:[\s,]+(?:for now|for today|later|soon|again|then|boss|bro|mate|dude|yaar|bhaijan|friend|man|thanks|thank you|sage))*[\s,.!?.-]*$/i;
